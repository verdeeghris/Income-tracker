        ...
        <div className="overflow-hidden">
          ...
          {filter === 'period' && isCalendarOpen && <div className="mx-4 mb-3 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 dark:border-emerald-400/20 dark:bg-emerald-400/[0.06] sm:mx-5">
            <div className="mb-2 flex items-center justify-between gap-2">
              <div className="sr-only" aria-live="polite">{range.from && !range.to ? 'Выберите конечную дату' : range.from && range.to ? 'Период выбран' : 'Выберите начальную дату'}</div>
            ...
            <p className="mb-2 text-center text-[11px] font-semibold text-emerald-800/75 dark:text-emerald-200/75">{range.from && !range.to ? 'Теперь выберите конечную дату' : range.from && range.to ? 'Период выбран' : 'Выберите начальную дату'}</p>
