        ...
        <div className="overflow-hidden">
          ...
          {filter === 'period' && isCalendarOpen && <div className="mx-4 mb-3 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 dark:border-emerald-400/20 dark:bg-emerald-400/[0.06] sm:mx-5">
            <div className="mb-2 flex items-center justify-between gap-2">
              ...
              {showYearPicker ? <select value={activeCalendarMonth.getFullYear()} onChange={(event) => { setCalendarMonth(new Date(Number(event.target.value), activeCalendarMonth.getMonth(), 1)); setShowYearPicker(false); }} aria-label="Выбрать год" className="rounded-md border border-emerald-200 bg-transparent px-1 py-0.5 text-xs font-extrabold dark:border-emerald-400/20"><option value={activeCalendarMonth.getFullYear() - 1}>{activeCalendarMonth.getFullYear() - 1}</option><option value={activeCalendarMonth.getFullYear()}>{activeCalendarMonth.getFullYear()}</option><option value={activeCalendarMonth.getFullYear() + 1}>{activeCalendarMonth.getFullYear() + 1}</option></select> : <button type="button" onClick={() => setShowYearPicker(true)} className="rounded-md px-1 py-0.5 text-xs font-extrabold capitalize hover:bg-emerald-100 dark:hover:bg-emerald-400/10">{monthLabel(calendarMonth)}</button>}
            ...
            <div className="mt-2 flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={() => { setIsCalendarOpen(false); setShowYearPicker(false); }} className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] font-bold text-emerald-700 hover:bg-emerald-100 dark:text-emerald-300 dark:hover:bg-emerald-400/10" aria-label="Закрыть календарь периода"><X className="h-3.5 w-3.5" />Закрыть</button>
            </div>
          </div>}
