...
export default function App() {
  ...
  const [projects, setProjects] = useState([]); const [lessons, setLessons] = useState([]); const [dataLoading, setDataLoading] = useState(false)
  ...
  const fetchData = useCallback(async () => { if (!session?.user || isMockSession) return; setDataLoading(true); setError(''); const [p, l] = await Promise.all([supabase.from('projects').select('*').eq('user_id', session.user.id), supabase.from('lessons').select('*').eq('user_id', session.user.id)]); if (p.error || l.error) { setError('Не удалось загрузить данные. Попробуйте обновить страницу.'); setDataLoading(false); return } setProjects((p.data || []).map((row) => normalize(row, 'project'))); setLessons((l.data || []).map((row) => normalize(row, 'lesson'))); setDataLoading(false) }, [session, isMockSession])
  ...
  const handleSave = async (data) => { if (!session?.user || !modal) return; setError(''); if (isMockSession) { const existingKeys = new Set(allInstances.filter((item) => item.type === modal.type && item.title === data.title.trim() && item.id !== modal.instance?.id).map((item) => `${item.type}:${item.title}:${item.date}`)); const dates = [...new Set([modal.dateKey, ...(data.copyDates || [])])].filter((date) => !existingKeys.has(`${modal.type}:${data.title.trim()}:${date}`)); const created = dates.map((date, index) => ({ id: modal.instance && index === 0 ? modal.instance.id : `mock-${Date.now()}-${index}`, type: modal.type, date, title: data.title, amount: Number(data.amount) || 0, color: data.color, isPaid: data.isPaid, isGph: data.isGph === true, instanceKey: `${modal.type}_${modal.instance && index === 0 ? modal.instance.id : `mock-${Date.now()}-${index}`}_${date}` })); if (modal.instance) { const update = (items) => items.map((item) => item.id === modal.instance.id ? { ...item, ...created[0] } : item); const addCopies = created.slice(1); modal.type === 'lesson' ? setLessons((items) => [...update(items), ...addCopies]) : setProjects((items) => [...update(items), ...addCopies]) } else { modal.type === 'lesson' ? setLessons((items) => [...items, ...created]) : setProjects((items) => [...items, ...created]) }; setModal(null); return } const table = modal.type === 'lesson' ? 'lessons' : 'projects'; const existingKeys = new Set(allInstances.filter((item) => item.type === modal.type && item.title === data.title.trim() && item.id !== modal.instance?.id).map((item) => `${item.type}:${item.title}:${item.date}`)); const dates = modal.instance ? [modal.dateKey] : [...new Set([modal.dateKey, ...(data.copyDates || [])])].filter((date) => !existingKeys.has(`${modal.type}:${data.title.trim()}:${date}`)); const payload = (date) => modal.type === 'lesson' ? { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, lesson_date: date, is_paid: data.isPaid } : { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, start_date: date, end_date: date, is_paid: data.isPaid, is_gph: data.isGph === true };
    const result = modal.instance ? await supabase.from(table).update(payload(modal.dateKey)).eq('id', modal.instance.id).eq('user_id', session.user.id) : await supabase.from(table).insert(dates.map(payload)).select('id'); if (result.error) { setError('Не удалось сохранить запись. Проверьте данные и попробуйте ещё раз.'); return } if (modal.instance && data.copyDates?.length) { const copies = [...new Set(data.copyDates)].filter((date) => date !== modal.dateKey && !existingKeys.has(`${modal.type}:${data.title.trim()}:${date}`)); const copyResult = await supabase.from(table).insert(copies.map(payload)).select('id'); if (copyResult.error) { setError('Запись сохранена, но копии создать не удалось. Попробуйте ещё раз.'); return } } await fetchData(); setModal(null) }
