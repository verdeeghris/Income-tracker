...
export default function App() {
  ...
  const templates = useMemo(() => { const seen = new Map(); [...projects, ...lessons].reverse().forEach((item) => { if (item.title && !seen.has(`${item.type}:${item.title}`)) seen.set(`${item.type}:${item.title}`, { title: item.title, color: item.color, amount: item.amount }) }); return [...seen.values()].slice(0, 8) }, [projects, lessons])
  ...
  const openDay = useCallback((dateKey) => { if ((byDate.get(dateKey) || []).length) setDayDetailsKey(dateKey); else openCreate(dateKey) }, [byDate, openCreate])
  const handleSave = async (data) => { if (!session?.user || !modal) return; setError(''); const table = modal.type === 'lesson' ? 'lessons' : 'projects'; const dates = modal.instance ? [modal.dateKey] : [...new Set([modal.dateKey, ...(data.copyDates || [])])]; const payload = (date) => modal.type === 'lesson' ? { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, lesson_date: date, is_paid: data.isPaid } : { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, start_date: date, end_date: date, is_paid: data.isPaid, is_gph: data.isGph === true };
    const result = modal.instance ? await supabase.from(table).update(payload(modal.dateKey)).eq('id', modal.instance.id).eq('user_id', session.user.id) : await supabase.from(table).insert(dates.map(payload)); if (result.error) { setError('Не удалось сохранить запись. Проверьте соединение и попробуйте ещё раз.'); return } await fetchData(); setModal(null) }
