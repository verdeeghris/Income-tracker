...
export default function App() {
  ...
  const handleSave = async (data) => { if (!session?.user || !modal) return; setError(''); if (isMockSession) { const dates = modal.instance ? [modal.dateKey] : [...new Set([modal.dateKey, ...(data.copyDates || [])])]; const created = dates.map((date, index) => ({ id: `mock-${Date.now()}-${index}`, type: modal.type, date, title: data.title, amount: Number(data.amount) || 0, color: data.color, isPaid: data.isPaid, isGph: data.isGph === true, instanceKey: `mock-${Date.now()}-${index}` })); if (modal.instance) { const update = (items) => items.map((item) => item.id === modal.instance.id ? { ...item, ...created[0] } : item); modal.type === 'lesson' ? setLessons(update) : setProjects(update) } else { modal.type === 'lesson' ? setLessons((items) => [...items, ...created]) : setProjects((items) => [...items, ...created]) }; setModal(null); return } const table = modal.type === 'lesson' ? 'lessons' : 'projects'; const dates = modal.instance ? [modal.dateKey] : [...new Set([modal.dateKey, ...(data.copyDates || [])])]; const payload = (date) => modal.type === 'lesson' ? { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, lesson_date: date, is_paid: data.isPaid } : { user_id: session.user.id, title: data.title, amount: data.amount, color: data.color, start_date: date, end_date: date, is_paid: data.isPaid, is_gph: data.isGph === true };
    const result = modal.instance ? await supabase.from(table).update(payload(modal.dateKey)).eq('id', modal.instance.id).eq('user_id', session.user.id).select('id').maybeSingle() : await supabase.from(table).insert(dates.map(payload)).select('id'); if (result.error) { setError(`Не удалось сохранить запись: ${result.error.message}`); return } if (modal.instance && !result.data) { setError('Запись не обновлена: Supabase не вернул строку. Проверьте user_id и RLS-политики.'); return } if (modal.instance && data.copyDates?.length) { const copies = [...new Set(data.copyDates)].filter((date) => date !== modal.dateKey); const copyResult = await supabase.from(table).insert(copies.map(payload)).select('id'); if (copyResult.error) { setError(`Основная запись сохранена, но копии не созданы: ${copyResult.error.message}`); return } } await fetchData(); setModal(null) }
